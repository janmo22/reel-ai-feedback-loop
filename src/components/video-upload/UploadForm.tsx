        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button type="submit" disabled={!title || isUploading || missions.length === 0 || !mainMessage} className="w-full sm:w-auto bg-flow-electric hover:bg-flow-electric/90 font-medium py-6 px-8 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 text-flow-dark bg-gray-300 hover:bg-gray-200">
                {isUploading ? <>
...
const MissionButton = ({
  ...
  return <button type="button" id={id} onClick={onClick} className={`flex flex-col items-start text-left p-3 rounded-md border transition-all duration-200 ${isSelected ? "border-flow-electric bg-flow-electric/10 shadow-sm" : "border-border/60 hover:border-flow-electric/60"}`}>
