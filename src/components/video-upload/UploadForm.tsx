    ...
    <form onSubmit={onSubmit} className="space-y-6">
      <div>
        ...
        <Input
          ...
          className="mt-1 font-satoshi border-border/60 focus:border-flow-electric transition-all"
        />
      </div>
      ...
      <div className="bg-muted/30 rounded-lg p-4 border border-border/40">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <MissionButton 
            id="mission-educate"
            isSelected={missions.includes('educar')}
            onClick={() => onMissionChange('educar')}
            title="Educar"
            description="Compartir conocimientos o enseñar algo nuevo"
          />
          
          <MissionButton 
            id="mission-entertain"
            isSelected={missions.includes('entretener')}
            onClick={() => onMissionChange('entretener')}
            title="Entretener"
            description="Divertir o captar la atención de la audiencia"
          />
          
          <MissionButton 
            id="mission-inspire"
            isSelected={missions.includes('inspirar')}
            onClick={() => onMissionChange('inspirar')}
            title="Inspirar"
            description="Motivar o transmitir un mensaje emocional"
          />
        </div>
      </div>
      ...
      {isUploading && (
        <div className="space-y-2 bg-muted/30 p-4 rounded-lg border border-border/40">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button 
                ...
                className="w-full sm:w-auto bg-flow-electric hover:bg-flow-electric/90 text-white font-medium py-6 px-8 rounded-lg shadow-md hover:shadow-lg transition-all duration-200"
              >
                {isUploading ? (
                  <>
            ...
            <TooltipContent side="top">
              {!title ? 'Ingresa un título' : 
               missions.length === 0 ? 'Selecciona al menos una misión' : 
               !mainMessage ? 'Ingresa el mensaje principal' : 
               'Enviar reel para análisis de IA'}
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      </div>
...
const MissionButton = ({ id, isSelected, onClick, title, description }) => {
  return (
    <button
      type="button"
      id={id}
      onClick={onClick}
      className={`flex flex-col items-start text-left p-3 rounded-md border transition-all duration-200 ${
        isSelected 
          ? "border-flow-electric bg-flow-electric/10 shadow-sm" 
          : "border-border/60 hover:border-flow-electric/60"
      }`}
    >
