  ...
  return <Sidebar variant="inset">
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {menuItems.map(item => <SidebarMenuItem key={item.label}>
                  {!item.hasSubmenu ? <SidebarMenuButton asChild isActive={isActive(item.path)} tooltip={item.description}>
                      <Link to={item.path} className="flex items-center gap-3">
                        <item.icon className="h-5 w-5" />
                        <span>{item.label}</span>
                      </Link>
                    </SidebarMenuButton> : <>
                      <SidebarMenuButton isActive={location.pathname === "/strategy"} tooltip={item.description} onClick={() => setStrategyOpen(!strategyOpen)} className="flex items-center justify-between w-full">
                      ...
                      {strategyOpen && item.submenu && <SidebarMenuSub>
                          {item.submenu.map(subItem => <SidebarMenuSubItem key={subItem.label}>
                              <SidebarMenuSubButton asChild isActive={isSubActive(subItem.path.split("=")[1])}>
      ...
      <SidebarFooter className="border-t p-4">
        <div className="flex flex-col gap-2">
          <Button variant="outline" size="sm" className="w-full justify-start text-muted-foreground" onClick={() => signOut()}>
