...
const funnyLoadingPhrases = [
  ...
  "Consultando con expertos digitales de todo el metaverso...",
...
const UploadPage = () => {
  ...
  const handleUploadComplete = (data: any) => {
    console.log("Upload completed, transitioning to processing:", data);
    setUploadData(data);
    ...
    // Navigate to results page immediately to show processing
    if (data.response && data.response.videoId) {
      navigate(`/results?videoId=${data.response.videoId}`, {
        state: {
  ...
  const handleContinue = () => {
    ...
    if (uploadData && uploadData.response && uploadData.response.videoId) {
      navigate(`/results?videoId=${uploadData.response.videoId}`, {
        state: {
